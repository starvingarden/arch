...
# configure mkinitcpio.conf (see arch wiki page "dm-crypt/Encrypting an entire system")
...
# add the encrypt hook before the filesystems hook for encryption support (see examples at /etc/mkinitcpio.conf, and arch wiki page "dm-crypt/System configuration#mkinitcpio")
...
# change the "udev" hook to the "systemd" hook
...
encryptedrootUUID=$(blkid -s UUID -o value "${rootPartitions[0]}")
# get first decrypted swap partition uuid
decryptedrootUUID=$(blkid -s UUID -o value "${decryptedrootPartitions[0]}")


# configure grub (see the  arch wiki pages)
#printf "\e[1;32m\nConfiguring grub\n\e[0m"
#sleep 3
...
# cryptdevice=UUID=$rootUUID:${decryptedrootpartitionNames[0]} (specifies the encrypted root partition for the encrypt hook)
# cryptkey=rootfs:/.crypt-keys/crypt-key.bin (uses the appropriate keyfile to unlock the root filesystem on boot using the encrypt hook)
# resume=UUID=$swapUUID (enables resuming from swap file hibernation)
# sysctl.vm.swappiness=0 (sets swappiness on boot)
#sed -i "s|GRUB_CMDLINE_LINUX_DEFAULT=\"|GRUB_CMDLINE_LINUX_DEFAULT=\"cryptdevice=UUID=$rootUUID:${decryptedrootpartitionNames[0]} cryptkey=rootfs:/.crypt-keys/cryp-key.bin resume=UUID=$swapUUID sysctl.vm.swappiness=0 |" /etc/default/grub
#if [ "$multiBoot" == true ]
#then
...
#####################################################################################################
# need to modify uuid's (check uuid's when booting from shutdown and when booting from hibernation)
#####################################################################################################
...
sed -i 's/#GRUB_ENABLE_CRYPTODISK=y/GRUB_ENABLE_CRYPTODISK=y/' /etc/default/grub
# add the following kernel parameters
# rd.luks.name=$encryptedswapUUID=${decryptedswappartitionNames[0]} (specifies unlocking and naming of the swap partition on boot)
# rd.luks.key=$encryptedswapUUID=/.crypt-keys/crypt-key.bin
# rd.luks.name=$encryptedrootUUID=${decryptedrootpartitionNames[0]} (specifies unlocking and naming of the root partition on boot)
# rd.luks.key=$encryptedrootUUID=/.crypt-keys/crypt-key.bin
...
# sysctl.vm.swappiness=0 (sets swappiness on boot)
sed -i "s|GRUB_CMDLINE_LINUX_DEFAULT=\"|GRUB_CMDLINE_LINUX_DEFAULT=\"rd.luks.name=$encryptedswapUUID=${decryptedswappartitionNames[0]} rd.luks.key=$encryptedswapUUID=/.crypt-keys/crypt-key.bin rd.luks.name=$encryptedrootUUID=${decryptedrootpartitionNames[0]} rd.luks.key=$encryptedrootUUID=/.crypt-keys/crypt-key.bin root=UUID=$decryptedrootUUID resume=UUID=$decryptedswapUUID sysctl.vm.swappiness=0 |" /etc/default/grub
